...
class TestProfileView:
    ...
    async def test_unauthenticated_user_cannot_view_oter_users_profile(
        self,
        app: FastAPI,
        client: AsyncClient,
        test_user2: UserInDB,
    ...
    async def test_no_profile_when_user_does_not_exist(
        self,
        app: FastAPI,
        authorized_client: AsyncClient,
    ) -> None:
        res = await authorized_client.get(
            app.url_path_for("profiles:get-profile-by-username", username="somename")
        )
        assert res.status_code == status.HTTP_404_NOT_FOUND
...
class TestProfileManagement:
    ...
    async def test_user_gets_error_for_invalid_params(
        self,
        app: FastAPI,
        authorized_client: AsyncClient,
        test_user: UserInDB,
