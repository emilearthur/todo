...
class TestCreateTodo:
    async def test_valid_input_create_todo(self, app: FastAPI,
                                           authorized_client: AsyncClient,
                                           test_user: UserInDB,
                                           new_todo: TodoCreate) -> None:
        res = await authorized_client.post(app.url_path_for("todos:create-todo"),
...
class TestGetTodo:
    ...
    async def test_unauthorized_users_cant_get_access_todos(self, app: FastAPI,
                                                            client: AsyncClient,
                                                            test_todo: TodoCreate) -> None:
    ...
    async def test_get_all_todos_returns_only_to_do_owned_todos(self, app: FastAPI,
                                                                authorized_client: AsyncClient,
                                                                test_user: UserInDB,
                                                                db: Database,
                                                                test_todo: TodoInDB,
                                                                test_todos_list: List[TodoInDB],) -> None:
        res = await authorized_client.get(app.url_path_for("todos:list-all-user-todos"))
        assert res.status_code == status.HTTP_200_OK
        assert isinstance(res.json(), list)
        assert len(res.json()) > 0
...
class TestUpdateTodo:
    ...
    async def test_update_todo_with_valid_input(self, app: FastAPI,
        ...
        res = await authorized_client.put(app.url_path_for("todos:update-todo-by-id", todo_id=test_todo.id),
    ...
    async def test_user_gets_error_if_updating_other_users_todo(self, app: FastAPI,
                                                                authorized_client: AsyncClient,
                                                                test_todos_list: List[TodoInDB],) -> None:
        res = await authorized_client.put(app.url_path_for("todos:update-todo-by-id", todo_id=test_todos_list[0].id),
...
class TestDeleteTodo:
    ...
    async def test_user_cannot_delete_others_todo(self, app: FastAPI,
                                                  authorized_client: AsyncClient,
                                                  test_todos_list: List[TodoInDB],) -> None:
