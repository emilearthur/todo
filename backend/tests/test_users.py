...
class TestUserRoutes:
    async def test_routes_exist(self, app: FastAPI,
        ...
        new_user = {"email": "test@gmail.com", "username": "test_user", "password": "testpassword"}
...
class TestUserRegistraton:
    async def test_users_can_register_successfully(self, app: FastAPI,
        ...
        new_user = {"email": "emilepxx@gmail.com", "username": "emilepxxxtrig", "password": "testpassword1"}
    ...
    async def test_user_registeration_fails_when_credentials_are_taken(self, app: FastAPI,
                                                                       client: AsyncClient,
                                                                       db: Database,
                                                                       attr: str,
                                                                       value: str,
    ...
    async def test_users_saved_password_is_hashed_and_has_salt(self, app: FastAPI,
        ...
        new_user = {"email": "frederickauthurxxx@hemile.io", "username": "fredeickauthuxxxr", "password": "gitpassword"}
        ...
        assert auth_service.verify_password(pwd=new_user["password"],
                                            salt=user_in_db.salt,
                                            hashed_pwd=user_in_db.password,)


class TestAuthTokens:
    async def test_can_create_access_token_successfully(self, app: FastAPI,
                                                        client: AsyncClient,
    ...
    async def test_can_get_username_from_token(self, app: FastAPI,
                                               client: AsyncClient,
                                               test_user: UserInDB) -> None:
    ...
    async def test_error_for_wrong_token_and_secret(self, app: FastAPI,
                                                    client: AsyncClient,
                                                    test_user: UserInDB,
...
class TestUserLogin:
    async def test_user_can_login_successfully_and_get_valid_token(self, app: FastAPI,
                                                                   client: AsyncClient,
                                                                   test_user: UserInDB,) -> None:
        client.headers["content-type"] = "application/x-www-form-urlencoded"
        login_data = {"username": test_user.email, "password": "mypassword", }
    ...
    async def test_user_with_wrong_creds_doesnt_get_token(self, app: FastAPI,
                                                          client: AsyncClient,
                                                          test_user: UserInDB,
                                                          credential: str,
                                                          wrong_value: str,
...
class TestUserMe:
    async def test_authenicated_user_can_get_own_data(self, app: FastAPI,
    ...
    async def test_unauthenitcated_user_cannot_acccess_own_data(self, app: FastAPI,
                                                                client: AsyncClient,
                                                                test_user: UserInDB,) -> None:
    ...
    async def test_user_with_incorrect_jwt_prefix_cannot_access_own_data(self, app: FastAPI,
                                                                         client: AsyncClient,
                                                                         test_user: UserInDB,
                                                                         jwt_prefix: str) -> None:
        token = auth_service.create_access_token_for_user(user=test_user, secret_key=str(SECRET_KEY))
