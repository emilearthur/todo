...
class CommentsRepository(BaseRepository):
    ...
    def __init__(self, db: Database) -> None:
        ...
        super().__init__(db)
        self.todos_repo = TodosRepository(db)
    ...
    async def create_comment(self, *, new_comment: CommentCreate, requesting_user: UserInDB) -> CommentInDB:
        ...
        if not todo_exist:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="todo cannot be found thus cannot comment")

        comment = await self.db.fetch_one(query=CREATE_COMMENT_QUERY,
                                          values={**new_comment.dict(), "comment_owner": requesting_user.id})
    ...
    async def get_todo_comments(self, *, todo: TodoInDB) -> List[CommentInDB]:
        ...
        comments = await self.db.fetch_all(query=GET_ALL_TODO_COMMENTS_QUERY,
    ...
    async def delete_comment(self, *, comment: CommentInDB) -> int:
        """Delete a comment."""
        delete_comment_id = await self.db.execute(query=DELETE_COMMENT_BY_ID_QUERY,
                                                  values={"id": comment.id})
        return delete_comment_id

    async def get_users_comments(self, *, requesting_user: UserInDB) -> List[CommentInDB]:
        """Get users comments."""
        comments = await self.db.fetch_all(query=GET_ALL_USER_COMMENTS_QUERY,
                                           values={"comment_owner": requesting_user.id})
        print(comments)
        return [CommentInDB(**comment) for comment in comments]
